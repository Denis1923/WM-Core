...
{
    public class LeasingContractAddAgreement : IOperation
    {
        public string GetContentType() => "leasingContract.additionalAgreement";
        ...
        public BaseResponse Execute(Dictionary<string, string> parameters)
        {
            try
            {
                ...
                var body = new Dictionary<string, string>
                {
                    ...
                    { "earchiveId", "GUID1С" },
                    { "parent", "GUIDContract" },
                    { "number", "НомерДопСоглашения" },
                    { "date", "ДатаДопСоглашенияЭА" },
